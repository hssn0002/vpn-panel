...
db.exec(`
  CREATE TABLE IF NOT EXISTS users (
    ...
    contact_type TEXT DEFAULT 'telegram',  -- 'telegram' or 'bale'
    contact_id TEXT DEFAULT '',
    subscription_links TEXT DEFAULT '[]',  -- JSON array of sub links
    unlimited_volume INTEGER DEFAULT 0,    -- 0=normal, 1=unlimited volume
    manual_days INTEGER DEFAULT 30,
    vless_links TEXT DEFAULT '[]',         -- JSON array of manual VLESS links
    ...
    sub_error INTEGER DEFAULT 0,          -- 0=OK, 1=error
    last_data TEXT DEFAULT '{}',          -- last known good data
...
const defaultSettings = {
  admin_password: '$2a$10$dummy_hash_for_427726', // will be set on first run
  site_url: '',
  ssl_cert: '',
  ssl_key: '',
  telegram_token: '',
...
function getUsers(sortBy = 'remaining_days', sortDir = 'ASC') {
  ...
  // Error users always on top
  return db.prepare(`
    SELECT * FROM users ORDER BY sub_error DESC, ${sortBy} ${sortDir}
  `).all();
}
...
function updateUser(id, data) {
  ...
  for (const [key, val] of Object.entries(data)) {
    if (['username','contact_type','contact_id','subscription_links','unlimited_volume',
         'manual_days','vless_links','remaining_volume','remaining_days','total_volume',
         'total_days','used_volume','last_checked','sub_error','last_data'].includes(key)) {
      fields.push(`${key} = @${key}`);
      values[key] = val;
    }
  }
...
function deleteUsers(ids) {
  ...
  return db.prepare(`DELETE FROM users WHERE id IN (${placeholders})`).run(...ids);
}
...
function addMessage(userId, senderType, message, image = '') {
  return db.prepare(`
    INSERT INTO messages (user_id, sender_type, message, image) VALUES (?, ?, ?, ?)
  `).run(userId, senderType, message, image);
}

function markMessagesSeen(userId) {
  return db.prepare('UPDATE messages SET seen = 1 WHERE user_id = ? AND sender_type = ? AND seen = 0')
    .run(userId, 'user');
}
